...
# RUNTIME_RTC_PREFIX_CONDITIONED
PREFIX_ATOL = 1.0e-3
...
def infer_batch_conditioned(
    policy: websocket_client_policy.WebsocketClientPolicy,
    images: np.ndarray,
    action_prefixes: np.ndarray,
) -> tuple[np.ndarray, float]:
    """Infer chunks conditioned on committed action prefixes."""

    images = np.asarray(images)
    action_prefixes = np.asarray(
        action_prefixes,
        dtype=np.float32,
    )
    ...
    if images.ndim != 4 or images.shape[1:] != IMAGE_SHAPE:
        raise RuntimeError(
            f"Expected batched images [N,{IMAGE_SHAPE}], "
            f"got {images.shape}"
        )
    if images.dtype != np.uint8:
        raise RuntimeError(
    ...
    if action_prefixes.ndim != 3:
        raise RuntimeError(
            "Expected action prefixes [N,P,A], "
            f"got {action_prefixes.shape}"
        )
    if action_prefixes.shape[0] != images.shape[0]:
        raise RuntimeError(
            "Image/prefix batch mismatch: "
            f"{images.shape[0]} vs {action_prefixes.shape[0]}"
        )
    if action_prefixes.shape[2] != ACTION_DIM:
        raise RuntimeError(
            f"Expected prefix action_dim={ACTION_DIM}, "
            f"got {action_prefixes.shape[2]}"
        )

    prefix_len = int(action_prefixes.shape[1])
    if not 1 <= prefix_len <= ACTION_HORIZON:
        raise RuntimeError(
            f"Invalid prefix length={prefix_len}"
        )
    if not np.isfinite(action_prefixes).all():
        raise RuntimeError(
            "Action prefix contains NaN or Inf"
        )

    chunks = []
    max_prefix_abs_error = 0.0

    for image, action_prefix in zip(
        images,
        action_prefixes,
        strict=True,
    ):
        ...
        max_prefix_abs_error = max(
            max_prefix_abs_error,
            prefix_abs_error,
        )
...
def main() -> None:
    ...
    if args.inference_delay == 0:
        raise ValueError(
            "Runtime RTC requires inference_delay > 0"
        )
    ...
    prefix_conditioned_model_calls = 0
    max_prefix_abs_error = 0.0
    start_time = time.time()

    for chunk_index in range(scan_length):
        current_images = render_images(
            render_batch,
            env_state,
        )

        committed_prefix = action_chunk[
            :,
        ...
        (
            ...
            current_prefix_abs_error,
        ) = infer_batch_conditioned(
            policy,
            current_images,
            committed_prefix,
        )

        prefix_conditioned_model_calls += args.num_evals
        max_prefix_abs_error = max(
            max_prefix_abs_error,
            current_prefix_abs_error,
        )

        if current_prefix_abs_error > PREFIX_ATOL:
            raise RuntimeError(
                "Runtime RTC prefix was not preserved: "
                f"error={current_prefix_abs_error:.8g}, "
                f"tolerance={PREFIX_ATOL:.8g}"
            )
        ...
        if (
            ...
            log(
                ...
                f"max_prefix_abs_error={max_prefix_abs_error:.8g}"
            )
    ...
    summary = {
        ...
        "prefix_conditioned_model_calls": int(
            prefix_conditioned_model_calls
        ),
        "prefix_length": int(args.inference_delay),
        "max_prefix_abs_error": float(
            max_prefix_abs_error
        ),
        "prefix_atol": float(PREFIX_ATOL),
        "runtime_rtc": True,
        ...
        "rollout_semantics": (
            "eval_flow_runtime_rtc_prefix_conditioned_host_side"
        ),
    }
